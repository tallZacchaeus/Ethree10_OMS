...
describe("Phase 6 core information architecture", () => {
  ...
  it("keeps a usable migration history", () => {
    // Without the lock file Prisma cannot read the folder as a history, which is
    // how the project ended up with no production migration path.
    expect(existsSync(resolve(process.cwd(), "prisma/migrations/migration_lock.toml"))).toBe(true);
    expect(existsSync(resolve(process.cwd(), "prisma/migrations/20260809000000_baseline/migration.sql"))).toBe(true);
  });
});
