  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-md border bg-background px-4 py-2 text-sm font-medium transition-colors hover:bg-muted"
      >
      ...
      {process.env.NODE_ENV === "development" && (
        <>
          ...
          <button
            type="button"
            onClick={async () => {
              const email = prompt("Enter email to login as:", "admin@ethree10.r4c.global");
              if (email) {
                await signIn("credentials", { email, redirectTo: "/dashboard" });
              }
            }}
            className="mt-4 flex w-full items-center justify-center gap-2 rounded-md border-2 border-dashed border-primary bg-primary/10 px-4 py-2 text-sm font-medium text-primary transition-colors hover:bg-primary/20"
          >
